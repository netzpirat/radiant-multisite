...
function boot_wym(elem) {
    ...
    jQuery(elem).wymeditor({
        ...
        dialogLinkHtml:  "<body class='wym_dialog wym_dialog_link'"
                + " onload='WYMeditor.INIT_DIALOG(" + WYMeditor.INDEX + "); loadPagesToChoose();'"
                + ">"
                + "<form>"
                + "<fieldset>"
                ...
                + "<legend>{Link}</legend>"
                + "<div class='row'>"
                + "<select id='link_page' size='40'/>{Choose}</select>"
                + "<label>{URL}</label><br/>"
        ...
        postInit: function(wym) {
            ...
            timers[elem.id] = setInterval(function() {
...
/**
 * Loads the pages for internal linking
 */
function loadPagesToChoose() {
    alert("yep");
}
